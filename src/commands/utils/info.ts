...
export default class InfoCommand extends BaseCommand {
    ...
    async run(ctx: CommandCTX) {
        ...
        const owner = BaseCommand.owners[0];
        ...
        const infoEmbed = new BaseCommand.discord.MessageEmbed({
            author: {
                name: ctx.client.user?.username || "Ririchiyo",
                iconURL: ctx.client.user?.avatarURL() || "https://cdn.discordapp.com/embed/avatars/4.png"
            },
            fields: [
                { name: "Version", value: BaseCommand.settings.info.version, inline: true },
                { name: "Library", value: BaseCommand.settings.info.library, inline: true },
                { name: "Made by", value: `${owner.username} [${await BaseCommand.getEmoji("animated_panda_happy")}](${owner.profileURL})`, inline: true },
            ...
            footer: {
                text: `${BaseCommand.settings.info.hosting} | Shard [${ctx.guild.shard.id + 1}/${ctx.client.shard?.count || 1}] | Uptime ${uptime}`
            },
